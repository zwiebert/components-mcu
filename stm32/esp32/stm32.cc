...
#define OUR_UART_NUM UART_NUM_1
...
int stm32_write(const char *data, unsigned data_len) {
  ...
  if (stm32_mode != STM32_MODE_FIRMWARE)
    return -1;
  return uart_write_bytes(OUR_UART_NUM, (const char*) data, data_len);
}
...
int stm32_read(char *buf, unsigned buf_size) {
  ...
  if (stm32_mode != STM32_MODE_FIRMWARE)
    return -1;
  return uart_read_bytes(OUR_UART_NUM, (uint8_t*) buf, buf_size, 20 / portTICK_PERIOD_MS);
}
...
int stm32_read_line(char *buf, unsigned buf_size, unsigned wait_ms) { // XXX: Implement parameter wait_ms, which is currently ignored
  uart_event_t event;
  ...
  while (uart0_queue) {

    {
      ...
      D(ESP_LOGI(TAG, "uart[%d] event:", OUR_UART_NUM));
      if (event.type == MY_UART_WAKE_UP)
        return 0;
      ...
      case UART_DATA: {
        ...
        uart_read_bytes(OUR_UART_NUM, dtmp, event.size, portMAX_DELAY);
        D(ESP_LOGI(TAG, "[DATA EVT]:"));
        uart_write_bytes(OUR_UART_NUM, (const char*) dtmp, event.size);
      }
        break;
      ...
      case UART_BUFFER_FULL: {
        ...
        int result = uart_read_bytes(OUR_UART_NUM, (uint8_t*) buf, buf_size, 100 / portTICK_PERIOD_MS);
        xQueueReset(uart0_queue);
        return result;
      }
        break;
      ...
      case UART_PATTERN_DET: {
        ...
        uart_get_buffered_data_len(OUR_UART_NUM, &buffered_size);
        int pos = uart_pattern_pop_pos(OUR_UART_NUM);
        ...
        if (pos == -1) {
          ...
          int result = uart_read_bytes(OUR_UART_NUM, dtmp, pos + PATTERN_CHR_NUM, 100 / portTICK_PERIOD_MS);
...
int stm32_getc(bool block) {
  ...
  uint8_t buf = 0;
  if (uart_read_bytes(OUR_UART_NUM, (uint8_t*) &buf, 1, block ? (20 / portTICK_PERIOD_MS) : 1) == 1)
    return buf;
...
// If a driver was installed then we need to delete it before doing anything
  if (stm32_mode != STM32_MODE_NONE) {
    ...
    uart0_queue = nullptr;
    uart_driver_delete(OUR_UART_NUM);
...
// The stm32 boot loader needs even parity
  ...
  if ((err = uart_param_config(OUR_UART_NUM, &uart_config))) {
    ESP_LOGE(TAG, "uart_param_config failed: %s", esp_err_to_name(err));
    return;
  }

  if ((err = uart_set_pin(OUR_UART_NUM, stm32_cfg->uart_tx_gpio, stm32_cfg->uart_rx_gpio, ECHO_TEST_RTS, ECHO_TEST_CTS))) {
  ...
  if ((err = uart_driver_install(OUR_UART_NUM, RX_BUF_SIZE, TX_BUF_SIZE, queue_size, queue, 0))) {
  ...
  if (mode == STM32_MODE_FIRMWARE) {
    ...
    uart_enable_pattern_det_baud_intr(OUR_UART_NUM, PATTERN_CHR, PATTERN_CHR_NUM, 9, 0, 0);
    //Reset the pattern queue length to record at most PATTERN_NUM_MAX pattern positions.
    uart_pattern_queue_reset(OUR_UART_NUM, PATTERN_NUM_MAX);
  }
